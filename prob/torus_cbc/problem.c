...
void init_prob()
{
  ...
  int status = EOS_SC_find_adiabat_1d(entropy, const_ye,
				      &adiabat);
  ...
  ZSLOOP(-1, N1, -1, N2, -1, N3) {
     ...
     * so it needs to be transformed at the end */
      ...
      hm1 = exp(lnh[i][j][k]) - 1.;
      #if EOS == EOS_TYPE_GAMMA || GAMMA_FALLBACK
      rho = pow(hm1 * (gam - 1.) / (kappa_eos * gam),
		1. / (gam - 1.));
      u = kappa_eos * pow(rho, gam) / (gam - 1.);
      #elif EOS == EOS_TYPE_TABLE
      hm1 += hm1_min;
      if (lrho0 < EOS_SC_get_min_lrho()) lrho0 = lrho_guess;
      ...
      up =
	(2. * a * r * sqrt(1. + up1 * up1) / sqrt(AA * SS * DD)
	 + sqrt(SS / AA) * up1 / sth);
  ...
  */
  ...
  ZSLOOP(-1, N1, -1, N2, -1, N3) {
    ...
    if ( r > rin && lnh[i][j][k] >= 0. ) {
      ...
      #endif
      press = EOS_pressure_rho0_u(PsaveLocal[i][j][k][RHO],
				  PsaveLocal[i][j][k][UU],
				  extra[i][j][k]);
  ...
  ZLOOP {
    ...
    if ( r > rin && lnh[i][j][k] >= 0. ) {
      ...
      if (ent > entmax) entmax = ent;
    }
  }
  entmax = mpi_max(entmax);
  if (mpi_io_proc()) {
    fprintf(stdout,
	    "Maximum entropy in disk = %e\n",
	    entmax);
  }
  #endif
  ...
  if (!do_bfield) {
    ...
    return;
  } // otherwise, do B field. Cblassic harm disk.
