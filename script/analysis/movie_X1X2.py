...
#FRAMEDIR = 'FRAMES'
FRAMEDIR = os.path.join(path, 'frames_X1X2/')
print FRAMEDIR
...
def plot(args):
  ...
  ax.pcolormesh(np.log10(dump['RHO'][:,:,0]).transpose(), vmin=-4, vmax=0, cmap='jet')
  ...
  ax.pcolormesh(np.log10(dump['beta'][:,:,0]).transpose(), vmin=-2, vmax=2, cmap='RdBu_r')
  ax.set_xlabel('i'); ax.set_ylabel('j')
  ...
  ax.pcolormesh(np.log10(dump['RHO'][:,hdr['N2']/2,:]).transpose(), vmin=-4, vmax=0, cmap='jet')
  ax.set_xlabel('i'); ax.set_ylabel('k')
  #bplt.plot_xy(ax, geom, np.log10(dump['RHO']), dump,
  #   vmin=-4, vmax=0, label='RHO')
  #ax.set_xlim([-SIZE, SIZE]); ax.set_ylim([-SIZE, SIZE])
  
  ax = plt.subplot(2,2,4)
  ax.pcolormesh(np.log10(dump['beta'][:,hdr['N2']/2,:]).transpose(), vmin=-2, vmax=2, cmap='RdBu_r')
  ax.set_xlabel('i'); ax.set_ylabel('k')
